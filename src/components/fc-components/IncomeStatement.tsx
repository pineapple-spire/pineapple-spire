...
const financialCategories = ['Revenue', 'Net Sales'];
...
const FCTable = () => {
  ...
  return (
    <Container>
      ...
      <Row className="m-3">
        <Col>
          <h3 style={{ fontWeight: '600', color: '#4e73df' }}>Income Statement</h3>
          <p className="text-muted" style={{ fontSize: '1rem' }}>
            Analyze the different types of income and their forecast.
          </p>
        </Col>
        <Col>
          ...
          <Table striped bordered>
            ...
            <tbody>
              {financialCategories.map((name) => (
                <tr key={name}>
                  <td>{name === 'Revenue' ? <ForecastTypeDropdown /> : null}</td>
                  <td>{name}</td>
                </tr>
              ))}
            </tbody>
